...
Liste9=['â', '7', 'd', '%', ' ', 'L', '9', '1', '&', 'J', 's', '/', 'O', 'ö', '#', "'", 'c', '6', '@', 'è', 'e', 'H', '(', 'G', 'p', 'y', '-', 'a', '}', 'C', 'u', 'z', '+', '`', '.', 'û', '-', '<', '^', 'T', 'ê', '$', 'î', '{', '3', 'é', 'I', ']', 'N', '2', 'W', 'ü', '~', 'A', 'D', 'n', 'F', 'ä', 'U', 'g', 'µ', 'q', ':', 'ë', '8', 'f', 'S', ')', '>', '°', 'P', '£', '_', '[', '=', 'v', 'ï', 'X', 'ù', 'l', '.', '/', 'k', '!', 'M', 'E', '¤', 'i', 'x', '*', '§', '4', 'R', '5', 'h', 'w', 'ç', 'ô', '²', 'B', 'r', 'Y', 'Z', 'j', 'V', 'à', 'Q', '|', '*', ';', 'm', 'o', 'K', 't', 'b']# les listes de 1 à 9
...
def cesardecrypt(encrypted_message):
    ...
    return decrypted_message# def cryptage# def
...
while boucle != "exit":
    ...
    if crypt_or_decrypt == "crypter":
        ...
        message = input("->")
        print ()
        print ("donné un mot de passe / clé de chiffrement que vous devrez retenir :")
        mdp = input("->")
        print ()
        ...
        print("message crypté :",rep)
        #print ()
        print("quelle nom voulez vous donner a votre image crypter ? (noubliez pas le .png à la fin)") #phrase à changer
        nom_de_limage = input("->")
        print()
        hide(str(rep), "honk.jpg")
        print ("votre image sera créer lors de la fermeture de gronz")
        print()
        print ("voulez-vous quitter gronz ou recommencer ? (répondre <exit> ou <reste>)")
        boucle = input("->")

    elif crypt_or_decrypt == "decrypter":
        print ("quelle est le nom de votre image ? (avec l'extention precisée à la fin) :")
        nom_de_limage =""
    ...
    elif crypt_or_decrypt.lower() == "jsp" or crypt_or_decrypt.lower() == "je ne sais pas":
        print ("mince")
        print ("dommage")
        print("...")
        print ("a+")
        ...
        print("réponce mal orthographiée")
